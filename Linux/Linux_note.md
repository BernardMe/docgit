...
ps命令是Process Status的缩写。ps命令用来列出系统中当前运行的那些进程。ps命令列出的是当前那些进程的快照，就是执行ps命令那个时刻的那些进程，如果想要动态的显示进程信息，就可以使用top命令。

要对进程进行检测和控制，首先必须要了解当前进程的情况，也就是需要查看当前进程，而ps命令就是最基本同时也是非常强大的进程查看命令。使用该命令可以确定有哪些进程正在运行和运行的状态、进程是否结束、进程有没有僵死、哪些进程占用了过多的资源等等。总之大部分信息都是可以通过执行该命令得到的。

ps为我们提供了进程的一次性的查看，它所提供的查看结果并不动态连续的;但如果想对进程时间监控，应该用top工具。

kill命令用于杀死进程。

#### linux上进程有5种状态：
运行(正在运行或在运行队列中等待)
终端(休眠中，受阻，在等待某个条件的形成或接收到信号)
不可终端(收到信号不唤醒和不可运行，进程必须等待直到有中断发生)
...
说明(各相关信息的意义)：
F 代表这个程序的旗帜(flag), 4代表使用者为super user
S 代表这个程序的状态(STAT)，关于各STAT的意义将在后面介绍
UID 程序被该UID所拥有
PID 就是这个程序的ID
PPID 则是其上级父程序的ID
C CPU使用的资源百分比
...
NI 这个是Nice值，详后介
ADDR 这个时kernel function，指出该程序在内存的那个部分。
SZ 使用掉的内存大小
WCHAN 目前这个程序是否正在运行当中，若为-表示正在运行
TTY 登入者的终端机位置
TIME 使用掉的CPU时间。
CMD 所下达的指令为何
在预设的情况下，ps仅会列出与目前所在的bash shell有关的PID而已，所以，当我使用ps -l的时候，只有三个PID
