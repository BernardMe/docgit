...
<!--                <value>classpath:dbConection.properties</value> -->
<!--                <value>classpath:wifidogcode.properties</value> -->
...
<c:forEach items = "${Gifts}" var = "gift">
    <tr style="background-color: FFFFFF; text-align: center;">
        <td class = "Able_Steven_td"><%=i++ %><input name = "gift_Id" type = "hidden" value = "${gift.id }" /><input type = "hidden" name = "gift_orderIndex" value = "${gift.orderIndex}"/><c:forEach items = "${gift.types}" var = "type"><input class = "typeIds" type = "hidden" value = "${type.id }"/></c:forEach></td>
        <td>${gift.orderIndex }</td>
        <td><img src = "gift_sendImg?id=${gift.id}" height = "50" width = "80"/></td>
        <td>${gift.giftNO }</td>
        <td>${gift.name}</td>
        <td>${gift.info}</td>
...
传参
giftNO:1
id:402830815f9fab0f015f9fc5648a0000
```java
if(giftId != null && !("".equals(giftId)))
    sql = "select * from kmh_gift where giftno = '" + giftNO + "' and id != '" + giftId + "'";
else
    sql = "select * from kmh_gift where giftno = '" + giftNO + "'";
```
...
先校验 uuid 在数据库中是否已存在
POST传参
titleIconId:402830815f9fab0f015f9fc5648a0000
...
GET传参
gift.id:402830815f9fe738015fa029ef7c0000
